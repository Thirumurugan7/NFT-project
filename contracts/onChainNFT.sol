        ...
        returns (string memory, uint,uint, uint )
    {
      ...
      uint256 combinedRandomness = uint256(keccak256(abi.encodePacked(block.timestamp,_tokenIds.current())));
       ...
       emit randomColreurn(randomFireCol);
        ...
        return (string(abi.encodePacked(baseURL, svgBase64Encoded)) , randombgCol,randomCol ,randomFireCol ) ;
    }
...
function supportsInterface(bytes4 interfaceId)
  ...
  returns (bool) {
    ...
    function formatTokenURI(string memory imageURI, string memory tokenNo, string memory bg, string memory body, string memory fire)
        public
        pure
            ...
            string(
                abi.encodePacked(
                    ...
                    Base64.encode(
                        bytes(
                            abi.encodePacked(
'{"name": "on-chain nft -',tokenNo,'",',' "description": "This id done by neevan.eth", "image":"',
                                imageURI,
                                                         '","traits":{"bg":"', bg, '","body":"', body, '","fire":"', fire, '"}}'
     ...
     /* Mints the token */
    function mint()payable public hasMintPermission {

         


    // Check if msg.value is equal to the expected value
    require(msg.value == mintPrice, "Incorrect amount sent for minting");

    _tokenIds.increment();
        uint256 newItemId = _tokenIds.current();
         (string memory imageURI, uint bg, uint body, uint fire) = svgToImageURI();
         string memory id = convertUintToString(newItemId);
         string memory bgid = convertUintToString(bg);
         string memory bodyid = convertUintToString(body);
         string memory fireid = convertUintToString(fire);
        string memory tokenURI = formatTokenURI(imageURI,id,bgid,bodyid,fireid);
